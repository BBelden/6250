...
int main (int argc, char *argv[])
{
	...
	const char jobs[]="jobs",kill[]="kill",fg[]="fg",bg[]="bg",shmalloc[]="shmalloc",shmdel[]="shmdel";
	const char *cmds[17];
	...
	cmds[15]="shmalloc"; cmds[16]="shmdel";
	struct nlist *np;
	...
	while (!done)
	{
		...
		else if(strcmp(cmdArray[0],shmalloc)==0)
		{
			char execCmd[100] = "ls /dev/shm | grep ";
			strcat(execCmd,cmdArray[1]);
			//printf("execCmd: %s\n",execCmd);
			std::string result = exec(execCmd); 

			if (result != "")
			{
				while(int(result[0]) < 33 || int(result[0]) > 126)
					result == result.substr(1,result.length()-1);
				//printf("result.length(): %d\n",result.length());
				while(int(result[result.length()-1]) < 33 || int(result[result.length()-1]) > 126)
				{
					std::string result2 = result.substr(0,result.length()-1);
					result = result2;
				}
				//printf("result.length(): %d\n",result.length());
				//printf("result: %s\n",result.c_str());
				...
				//	printf("\t%c\n",result[p]);
				//printf("cmdArray[1]: %s\n",cmdArray[1]);
				//for (int p = 0; cmdArray[1][p] != '\0'; p++)
				//	printf("\t%c\n",cmdArray[1][p]);
				//printf("result.length(): %d\n",result.length());
				if (strcmp(result.c_str(),cmdArray[1])==0)
				{
					printf("shm object \"%s\" already exists\n",cmdArray[1]);
					continue;
				}
			}
			void *shmptr;
			unsigned int mode;
			int shmfd,SHM_SIZE;
			SHM_SIZE = atoi(cmdArray[2])*1024*1024;

			// open shmem object
			mode = S_IRWXU | S_IRWXG;
			shmfd = shm_open(cmdArray[1], O_CREAT|O_RDWR|O_TRUNC,mode);
			if (shmfd == -1)
			{
		...
		else if(strcmp(cmdArray[0],shmdel)==0)
		{
			shm_unlink(cmdArray[1]);
		} // end else if
